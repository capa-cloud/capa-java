...
//        CapaTelemetryClient client = buildFromBuilder();
    ...
    private static void metricsDemo(CapaTelemetryClient client) throws InterruptedException {
        Meter meter = client.buildMeter("demo-meter").block();

        LongCounter counter = meter.counterBuilder("simple-counter")
                                   .setDescription("simple add")
                                   .build();

        BoundLongCounter boundLongCounter1 = counter.bind(Attributes.of(AttributeKey.stringKey("type"), "attr-test-1"));

        LongUpDownCounter upDownCounter = meter.upDownCounterBuilder("simple-counter-with-dec")
                                               .build();
        DoubleHistogram doubleHistogram = meter.histogramBuilder("histogram")
                                               .build();

        for (int i = 0; i < 100; i++) {
            counter.add(10);
            boundLongCounter1.add(1);
            upDownCounter.add(10L);
            upDownCounter.add(-3);
            doubleHistogram.record(1.2);
            Thread.sleep(1000);
        }
    }
     ...
     */
    private static void traceDemo(CapaTelemetryClient client) throws InterruptedException {
